            ...
            <div className="flex items-center gap-4 mb-5 lg:mb-6">
              <Link href="/" className="shrink-0">
                <Image 
                  ...
                  width={140} 
                  height={40} 
                  className="h-8 lg:h-10 w-auto brightness-0" 
                />
              </Link>
              <p className="text-gray-500 text-caption max-w-[180px] lg:max-w-[200px]">
                Let&apos;s talk about your next milestone—and how to reach it
              </p>
            </div>
            <Button 
              onClick={openModal} 
              className="bg-primary hover:bg-primary-hover text-white text-button px-5 py-2.5 rounded-lg flex items-center w-max"
            >
          ...
          <div className="lg:w-3/4 grid grid-cols-2 lg:grid-cols-5 gap-x-4 gap-y-8 lg:gap-8">
            {FOOTER_NAV_COLUMNS.map((col) => (
              <div key={col.id} className={col.orderClasses}>
                <h4 className="text-black text-eyebrow mb-4 lg:mb-5">{col.title}</h4>
                <ul className="space-y-3 lg:space-y-4">
                  {col.links.map((link, idx) => (
                    <li key={idx}>
                      <Link href={link.href} className="text-gray-600 hover:text-black text-body-sm">
                        {link.label}
                      </Link>
                    </li>
                  ))}
