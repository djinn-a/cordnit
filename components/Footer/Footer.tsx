...
import { footerColumns, footerContacts, footerLegal } from '../../data/footer';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-gray-400 mb-6 max-w-sm leading-relaxed">
              We help individuals and businesses make confident decisions through thoughtful strategies, trusted expertise and practical solutions.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-4">
              {footerContacts.map((contact) => (
                <li key={contact.id} className="flex items-start text-gray-400">
                  {contact.type === 'email' && <Mail className="w-5 h-5 mr-3 mt-0.5 flex-shrink-0" />}
                  {contact.type === 'phone' && <Phone className="w-5 h-5 mr-3 mt-0.5 flex-shrink-0" />}
                  {contact.type === 'address' && <MapPin className="w-5 h-5 mr-3 mt-0.5 flex-shrink-0" />}
        ...
        <div className="border-t border-gray-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-6 text-sm">
            {footerLegal.map((link) => (
              <Link key={link.id} href={link.href} className="text-gray-500 hover:text-white transition-colors">
                {link.label}
              </Link>
            ))}
          </div>
