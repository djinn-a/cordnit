...
export default function PageLayout({ children, className }: Readonly<PageLayoutProps>) {
  return (
    <div className={cn("flex flex-col min-h-screen font-sans bg-surface items-center", className)}>
      <main className="grow flex flex-col w-full max-w-container-1440 px-space-16 md:px-space-32 lg:px-space-60 pt-space-24 pb-space-80 gap-space-56 md:gap-space-80 bg-surface">{children}</main>
    </div>
  );
}
